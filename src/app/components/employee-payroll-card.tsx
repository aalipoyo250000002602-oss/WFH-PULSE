...
import { PayrollInfo, Deduction, OtherEarning } from './employee-data'
...
export function EmployeePayrollCard({
    ...
    const [visibleDeductions, setVisibleDeductions] = useState<Set<string>>(
        new Set()
    )
    ...
    const [showDeleteEarningDialog, setShowDeleteEarningDialog] =
        useState(false)
    const [showEditDeductionDialog, setShowEditDeductionDialog] =
        useState(false)
    ...
    const [salaryAmount, setSalaryAmount] = useState(payroll?.salary ?? 0)
    const [editingEarning, setEditingEarning] = useState<OtherEarning | null>(
        null
    )
    ...
    const handleUpdateGovIds = () => {
        if (
            !govIdFormData.pagIbig ||
            !govIdFormData.philHealth ||
            !govIdFormData.sss ||
            !govIdFormData.tin
        ) {
    ...
    const handleDeleteDeduction = () => {
        ...
        const updatedDeductions = payroll.deductions.filter(
            d => d.id !== deductionToDelete
        )
    ...
    const handleUpdateSalary = () => {
        if (!Number.isFinite(salaryAmount) || salaryAmount < 0) {
            toast.error('Please provide a valid salary amount')
            return
        }

        onUpdate({ ...payroll, salary: salaryAmount })
        setShowEditSalaryDialog(false)
    }

    const openAddEarningDialog = () => {
        setEditingEarning(null)
        setEarningFormData({ name: '', amount: 0 })
        setShowEarningDialog(true)
    }

    const openEditEarningDialog = (earning: OtherEarning) => {
        setEditingEarning(earning)
        setEarningFormData({ name: earning.name, amount: earning.amount })
        setShowEarningDialog(true)
    }

    const handleSaveEarning = () => {
        const name = earningFormData.name.trim()
        if (!name || earningFormData.amount <= 0) {
            toast.error('Please provide a valid earning name and amount')
            return
        }

        const currentEarnings = payroll.otherEarnings ?? []
        const otherEarnings = editingEarning
            ? currentEarnings.map(earning =>
                  earning.id === editingEarning.id
                      ? { ...earning, name, amount: earningFormData.amount }
                      : earning
              )
            : [
                  ...currentEarnings,
                  {
                      id: `earning-custom-${Date.now()}`,
                      name,
                      amount: earningFormData.amount,
                  },
              ]

        onUpdate({ ...payroll, otherEarnings })
    ...
    const handleDeleteEarning = () => {
        ...
        onUpdate({
            ...payroll,
            otherEarnings: (payroll.otherEarnings ?? []).filter(
                earning => earning.id !== earningToDelete
            ),
        })
        setShowDeleteEarningDialog(false)
    ...
    const totalOtherEarnings = (payroll.otherEarnings ?? []).reduce(
        (sum, earning) => sum + earning.amount,
        0
    )
    const grossSalary = payroll.salary + totalOtherEarnings
    ...
    return (
        <>
                        ...
                        <CardContent className="space-y-6">
                                    ...
                                    {canEdit && (
                                        <Button
                                            ...
                                            onClick={() => {
                                                setGovIdFormData({
                                                    pagIbig:
                                                        payroll.governmentIds
                                                            .pagIbig,
                                                    philHealth:
                                                        payroll.governmentIds
                                                            .philHealth,
                                                    sss: payroll.governmentIds
                                                        .sss,
                                                    tin: payroll.governmentIds
                                                        .tin,
                                                })
                                ...
                                <div className="flex items-center justify-between mb-3">
                                    ...
                                    {canEdit && (
                                        <Button
                                            ...
                                            size="icon"
                                            onClick={() => {
                                                setSalaryAmount(payroll.salary)
                                ...
                                <div className="space-y-3">
                                    ...
                                    <div className="flex items-center justify-between gap-2">
                                        ...
                                        <Button
                                            ...
                                            size="icon"
                                            onClick={() =>
                                                setShowDailyRate(!showDailyRate)
                                            }
                                            className="flex-shrink-0"
                                        >
                                            {showDailyRate ? (
                            ...
                            <div>
                                ...
                                <div className="space-y-2">
                                    {(payroll.otherEarnings ?? []).length ===
                                    0 ? (
                                        <p className="text-sm text-muted-foreground">
                                            No other earnings added.
                                        </p>
                                    ) : (
                                        (payroll.otherEarnings ?? []).map(
                                            earning => (
                                                <div
                                                    key={earning.id}
                                                    className="flex items-center justify-between gap-2 p-3 rounded-lg bg-vibrant-green/10"
                                                >
                                                    <div className="min-w-0">
                                                        <p className="text-sm truncate">
                                                            {earning.name}
                                                        </p>
                                                        <p className="text-sm font-medium text-vibrant-green">
                                                            +{' '}
                                                            {formatCurrency(
                                                                earning.amount
                                                            )}
                                                        ...
                                                        <div className="flex gap-1">
                                                            ...
                                                            <Button
                                                                ...
                                                                onClick={() => {
                                                                    ...
                                                                    setShowDeleteEarningDialog(
                                                                        true
                                                                    )
                                                                }}
                                                                aria-label={`Delete ${earning.name}`}
                                                            >
            ...
            >
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Update this employee's base monthly salary.
                        </DialogDescription>
                    </DialogHeader>
                    <div className="space-y-2">
                    ...
                    <DialogFooter>
                        <Button
                            variant="outline"
                            onClick={() => setShowEditSalaryDialog(false)}
                        >
                            Cancel
                        </Button>
            ...
            <Dialog
                ...
                onOpenChange={setShowEarningDialog}
            >
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>
                        ...
                        <DialogDescription>
                            Add bonuses, allowances, commissions, or other
                            compensation.
                        </DialogDescription>
                    </DialogHeader>
                    <div className="space-y-4">
                        <div className="space-y-2">
                            <Label htmlFor="earning-name">Earning Name</Label>
                            <Input
                                id="earning-name"
                                value={earningFormData.name}
                                onChange={event =>
                                    setEarningFormData({
                                        ...earningFormData,
                    ...
                    <DialogFooter>
                        <Button
                            variant="outline"
                            onClick={() => setShowEarningDialog(false)}
                        >
                            Cancel
                        </Button>
            ...
            <AlertDialog
                open={showDeleteEarningDialog}
                onOpenChange={setShowDeleteEarningDialog}
            >
                <AlertDialogContent>
                    <AlertDialogHeader>
                        <AlertDialogTitle>
                            Delete Other Earning
                        </AlertDialogTitle>
                        <AlertDialogDescription>
                            This earning will be removed from the employee's
                            payroll.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
            ...
            <Dialog
                open={showEditGovIdDialog}
                onOpenChange={setShowEditGovIdDialog}
            >
                <DialogContent>
                    <DialogHeader>
                    ...
                    <DialogFooter>
                        <Button
                            variant="outline"
                            onClick={() => setShowEditGovIdDialog(false)}
                        >
                            Cancel
                        </Button>
                        <Button
                            onClick={handleUpdateGovIds}
            ...
            <Dialog
                open={showEditDeductionDialog}
                onOpenChange={setShowEditDeductionDialog}
            >
                <DialogContent>
                    <DialogHeader>
                    ...
                    <div className="space-y-4">
                        <div>
                            <Label htmlFor="edit-deduction-name">
                                Deduction Name *
                            </Label>
                            <Input
                        ...
                        <div>
                            <Label htmlFor="edit-deduction-amount">
                                Amount *
                            </Label>
                            <Input
                    ...
                    <DialogFooter>
                        <Button
                            variant="outline"
                            onClick={() => setShowEditDeductionDialog(false)}
                        >
                            Cancel
                        </Button>
                        <Button
            ...
            >
                <DialogContent>
                    ...
                    <div className="space-y-4">
                        <div>
                            <Label htmlFor="add-deduction-name">
                                Deduction Name *
                            </Label>
                            <Input
                        ...
                        <div>
                            <Label htmlFor="add-deduction-amount">
                                Amount *
                            </Label>
                            <Input
                    ...
                    <DialogFooter>
                        <Button
                            variant="outline"
                            onClick={() => setShowAddDeductionDialog(false)}
                        >
                            Cancel
                        </Button>
                        <Button
            ...
            >
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            Are you sure you want to delete this deduction? This

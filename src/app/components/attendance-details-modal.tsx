...
export function AttendanceDetailsModal({
    ...
    const parseOvertimeValuesFromLog = (reason: string | null | undefined) => {
        ...
        const startTime =
            text.match(/Start Time:\s*([^\n\r]+)/i)?.[1]?.trim() ?? ''
        const endTime = text.match(/End Time:\s*([^\n\r]+)/i)?.[1]?.trim() ?? ''
        const duration =
            text.match(/OT Duration:\s*([^\n\r]+)/i)?.[1]?.trim() ?? ''
    ...
    const parsedOvertimeFallback = parseOvertimeValuesFromLog(
        latestOvertimeLogWithValues?.reason
    )
    const displayOvertimeStartTime =
        overtimeRequest?.startTime?.trim() ||
        parsedOvertimeFallback.startTime ||
        '-'
    const displayOvertimeEndTime =
        overtimeRequest?.endTime?.trim() ||
        parsedOvertimeFallback.endTime ||
        '-'
    ...
    const displayOvertimeDuration =
        computedOvertimeDuration || parsedOvertimeFallback.duration || '-'
    const displayOvertimePurpose =
        overtimeRequest?.purpose?.trim() ||
        parsedOvertimeFallback.purpose ||
        '-'
    ...
    const getAdjustmentStatusBadge = (
        status: 'pending' | 'approved' | 'denied' | 'cancelled'
    ) => {
        switch (status) {
            case 'pending':
    ...
    const getRequestStateBadge = (
        label: string,
        status:
            'pending' | 'approved' | 'denied' | 'cancelled' | null | undefined
    ) => {
        if (!status) {
            return null
        }
        ...
        if (status === 'approved') {
            className =
                'bg-vibrant-green/10 text-vibrant-green border-vibrant-green/30'
            ...
            <div className="space-y-4 pt-4 border-t">
                ...
                <div className="space-y-2">
                    ...
                    <div className="space-y-2">
                        ...
                        {(adjustmentRequest.logTrail ?? []).map(
                            (log, index) => (
                                <div
            ...
            <div className="space-y-4 pt-4 border-t">
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1">
                        ...
                        <div className="font-medium">
                            {format(
                                new Date(overtimeRequest.requestDate),
                                'MMM dd, yyyy'
                            )}
                        </div>
                    </div>
        ...
        <div className="space-y-4">
            <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-2">
                    ...
                    {getRequestStateBadge(
                        'Adjustment',
                        details.adjustmentApprovalStatus ?? null
                    )}
                    {getRequestStateBadge(
                        'Overtime',
                        details.overtimeApprovalStatus ?? null
                    )}
                </div>
            </div>
            ...
            <div className="space-y-1">
                ...
                <div>
                    {details.lateMinutes && details.lateMinutes > 0 ? (
                        <Badge
                            variant="outline"
                            className="bg-vibrant-orange/10 text-vibrant-orange border-vibrant-orange/30"
                        >
        ...
        return (
            <div className="space-y-4">
                <div className="space-y-1">
                    ...
                    <div className="font-medium">
                        {format(
                            new Date(details.leaveDetails.requestDate),
                            'MMM dd, yyyy'
                        )}
                    </div>
                </div>
                ...
                <div className="space-y-1">
                    ...
                    <div className="font-medium">
                        {format(
                            new Date(details.leaveDetails.fromDate),
                            'MMM dd, yyyy'
                        )}{' '}
                        -{' '}
                        {format(
                            new Date(details.leaveDetails.toDate),
                            'MMM dd, yyyy'
                        )}
                    </div>
                </div>
                ...
                {details.leaveDetails.attachments.length > 0 && (
                    <div className="space-y-2">
                        ...
                        <div className="space-y-1">
                            {details.leaveDetails.attachments.map(
                                (attachment, index) => (
                                    <div
                                        key={index}
        ...
        switch (details.status) {
            ...
            case 'on-leave':
                return (
                    <Badge
                        variant="outline"
                        className="bg-vibrant-purple/10 text-vibrant-purple border-vibrant-purple/30"
                    >
                        On Leave
                    </Badge>
                )

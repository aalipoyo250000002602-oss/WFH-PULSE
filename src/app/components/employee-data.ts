...
const getEmploymentTypeNamesFromOptions = (
	employmentTypes: string[]
): string[] => {
	const seen = new Set<string>()
	const names: string[] = []
...
export const syncEmployeesWithEmploymentOptions = (
	...
	const employmentTypeNames = getEmploymentTypeNamesFromOptions(
	...
	return employees.map((employee, index) => {
		...
		const departmentPositions =
			positionNamesByDepartment.get(syncedDepartment.toLowerCase()) ?? []
		...
		const syncedPosition =
			...
			(!currentPosition ||
				!validPositionNames.has(currentPosition.toLowerCase()))
...
export const generateEmployees = (): Employee[] => {
	...
	const addresses = ['123 Uso St., Toril, Davao City, 8000, Philippines']
	...
	return Array.from({ length: 30 }, (_, i) => {
		...
		return {
			...
			employmentStatus,
			employmentType:
				i % 5 === 0 ? 'independent contractor' : 'full-time',
			clockInTime:
				status === 'present'
					? clockInTimes[i % clockInTimes.length]
					: undefined,
			clockOutTime: hasLeftWork
				? clockOutTimes[i % clockOutTimes.length]
				: undefined,
			workDuration:
				status === 'on-leave'
					? '8h 0m'
					: hasLeftWork
						? workDurations[i % workDurations.length]
						: undefined,
			lateMinutes,
			department,
			...
			invitationSentDate:
				employmentStatus === 'onboarding' && i === 27
					? '2025-10-15'
					: undefined,
			passwordChanged:
				employmentStatus === 'onboarding' && i === 28
					? true
					: undefined,
			payroll: payrollInfo,
...
export const addEmployee = (
	employee: Omit<Employee, 'id' | 'employeeId'>
): Employee => {

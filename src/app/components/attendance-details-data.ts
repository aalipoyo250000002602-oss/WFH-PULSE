...
const generateNovemberAttendance = (
	...
	for (let day = 1; day <= 15; day++) {
		...
		// Regular present day
			...
			const randomMinute =
				Math.floor(seededRandom(employeeIndex * 100 + day) * 15) + 45 // 45-59 minutes
			const clockInTime = `08:${String(randomMinute).padStart(2, '0')}`
			const clockOutMinute = Math.floor(
				seededRandom(employeeIndex * 200 + day) * 20
			) // 0-19 minutes
...
const generateJuneAttendance = (
	...
	let finalAbsentDate = absentDate
	while (usedDates.has(finalAbsentDate))
		finalAbsentDate = (finalAbsentDate % 19) + 1
	usedDates.add(finalAbsentDate)
	let finalLeaveDate = leaveDate
	while (usedDates.has(finalLeaveDate))
		finalLeaveDate = (finalLeaveDate % 19) + 1
	...
	for (let day = 1; day <= 30; day++) {
		...
		if (day === finalAbsentDate) {
			...
			const lateMinutes =
				30 +
				Math.floor(seededRandom(employeeIndex * 50 + day + 600) * 30)
			const clockInHour = 9 + Math.floor(lateMinutes / 60)
			const clockInMinute = lateMinutes % 60
			attendance[dateStr] = {
				date: dateStr,
			...
			const randomMinute =
				Math.floor(seededRandom(employeeIndex * 100 + day + 600) * 15) +
				45
			const clockOutMinute = Math.floor(
				seededRandom(employeeIndex * 200 + day + 600) * 20
			)
			attendance[dateStr] = {
				date: dateStr,
				status: 'present',
...
const generateJulyAttendance = (
	...
	let finalAbsentDate = absentDate
	while (usedDates.has(finalAbsentDate))
		finalAbsentDate = (finalAbsentDate % 13) + 1
	...
	for (let day = 1; day <= 21; day++) {
		...
		if (day === finalAbsentDate) {
			...
			const lateMinutes =
				30 +
				Math.floor(seededRandom(employeeIndex * 50 + day + 700) * 30)
			const clockInHour = 9 + Math.floor(lateMinutes / 60)
			const clockInMinute = lateMinutes % 60
			attendance[dateStr] = {
				date: dateStr,
			...
			const randomMinute =
				Math.floor(seededRandom(employeeIndex * 100 + day + 700) * 15) +
				45
			const clockOutMinute = Math.floor(
				seededRandom(employeeIndex * 200 + day + 700) * 20
			)
			attendance[dateStr] = {
				date: dateStr,
				status: 'present',
...
export const getAttendanceDetailsForEmployee = (
	employeeIndex: number
): Record<string, AttendanceDetails> => {
...
export const getInitialAdjustmentRequests =
	(): AttendanceAdjustmentRequest[] => [
		{
			...
			message:
				'I forgot to clock in this morning due to urgent meeting. I arrived at 9:00 AM and left at 5:00 PM.',
		...
		},
		{
			...
			message:
				'System malfunction prevented proper clock-in/out logging. I have email timestamps as proof.',

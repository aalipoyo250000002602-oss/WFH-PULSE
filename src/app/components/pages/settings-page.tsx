...
interface SettingsPageProps {
    ...
    leaveCreditTypes: Array<{
        leaveTypeId: string
        name: string
        defaultLimitDays: number
    }>
    canEditLeaveCreditTypes: boolean
    onUpdateLeaveCreditTypes: (
...
export function SettingsPage({
    ...
    useEffect(() => {
        setLeaveCreditFormData(
            leaveCreditTypes.map(type => ({
                leaveTypeId: type.leaveTypeId,
                name: type.name,
                defaultLimitDays: Math.max(
                    0,
                    Number(type.defaultLimitDays ?? 0)
                ),
            }))
        )
    }, [leaveCreditTypes])
    ...
    const [biometricLogin, setBiometricLogin] = useState(
        securityPreferences.biometricLogin
    )
    const [biometricClockIn, setBiometricClockIn] = useState(
        securityPreferences.biometricClockInOut
    )
    const [passwordWaived, setPasswordWaived] = useState(
        securityPreferences.passwordWaived
    )
    ...
    const [isLeaveCreditSaving, setIsLeaveCreditSaving] = useState(false)

    // Collapsible card states
    const [isProfileOpen, setIsProfileOpen] = useState(false)
    const [isLeaveCreditsOpen, setIsLeaveCreditsOpen] = useState(false)
    const [showEditLeaveCreditsDialog, setShowEditLeaveCreditsDialog] =
        useState(false)
    const [leaveCreditFormData, setLeaveCreditFormData] = useState<
        Array<{
            leaveTypeId: string
            name: string
            defaultLimitDays: number
    ...
    const [showEditEmploymentDialog, setShowEditEmploymentDialog] =
        useState(false)
    const [showEditGovernmentIdDialog, setShowEditGovernmentIdDialog] =
        useState(false)
    ...
    const handleUpdateEmployment = async () => {
        ...
        const selectedPosition = employmentOptions.positions.find(
            position =>
                String(position.positionId) === employmentFormData.positionId
        )
    ...
    const handlePasswordWaivedChange = async (checked: boolean) => {
        ...
        try {
            ...
            if (didUpdate) {
                toast.success(
                    `Password waive flag ${checked ? 'enabled' : 'disabled'}`
                )
            } else {
    ...
    const handleSaveLeaveCreditTypes = async () => {
        if (!canEditLeaveCreditTypes) {
            toast.error('Only admin users can edit leave credits')
            return
        }

        const hasInvalid = leaveCreditFormData.some(
            row =>
                !Number.isInteger(row.defaultLimitDays) ||
                row.defaultLimitDays < 0
        )
        if (hasInvalid) {
            toast.error('Leave credits must be non-negative integers')
            return
        }

        if (isLeaveCreditSaving) {
            return
        }

        setIsLeaveCreditSaving(true)
        try {
            const didUpdate = await onUpdateLeaveCreditTypes(
                leaveCreditFormData.map(row => ({
                    leaveTypeId: row.leaveTypeId,
                    defaultLimitDays: row.defaultLimitDays,
                }))
            )
            ...
            toast.success('Leave credit defaults updated successfully')
            setShowEditLeaveCreditsDialog(false)
        } finally {
            setIsLeaveCreditSaving(false)
        }
    }
    ...
    return (
        <div className="space-y-6 pb-20">
            <div className="px-4 space-y-4">
                                        ...
                                        <Button
                                            ...
                                            onClick={() => {
                                                setEmploymentFormData({
                                                    ...buildEmploymentFormData(
                                                        localProfile
                                                    ),
                                                })
                                                setShowEditEmploymentDialog(
                                                    true
                                                )
                                            }}
                                        >
                                        ...
                                        <Button
                                            ...
                                            onClick={() => {
                                                setGovernmentIdFormData({
                                                    ...buildGovernmentIdFormData(
                                                        localProfile
                                                    ),
                                                })
                                                setShowEditGovernmentIdDialog(
                                                    true
                                                )
                                            }}
                                        >
                ...
                {/* Leave Credits - Collapsible */}
                <Card>
                    <CardHeader
                        className="pb-3 cursor-pointer hover:bg-accent/50 transition-colors"
                        onClick={() =>
                            setIsLeaveCreditsOpen(!isLeaveCreditsOpen)
                        }
                    >
                        <div className="flex items-center justify-between">
                            <CardTitle className="flex items-center gap-2">
                                <CreditCard className="h-5 w-5 text-vibrant-blue" />
                                Leave Credits
                            </CardTitle>
                            <motion.div
                                animate={{
                                    rotate: isLeaveCreditsOpen ? 180 : 0,
                                }}
                                transition={{
                                    duration: 0.3,
                                    ease: 'easeInOut',
                ...
                <Card>
                    ...
                    >
                        <div className="flex items-center justify-between">
                            ...
                            <motion.div
                                animate={{
                                    rotate: isWorkingHoursOpen ? 180 : 0,
                                }}
                                transition={{
                                    duration: 0.3,
                                    ease: 'easeInOut',
                    ...
                    <div className="space-y-4">
                        <div>
                            ...
                            >
                                ...
                                <SelectContent>
                                    {employmentOptions.employmentTypes.map(
                                        employmentType => (
                                            <SelectItem
                                                key={employmentType}
                        ...
                        <div>
                            ...
                            >
                                ...
                                <SelectContent>
                                    {employmentOptions.departments.map(
                                        department => (
                                            <SelectItem
            ...
            <Dialog
                open={showEditGovernmentIdDialog}
                onOpenChange={setShowEditGovernmentIdDialog}
            >
                <DialogContent>
                    <DialogHeader>
                    ...
                    <div className="space-y-4">
                        ...
                        <div>
                            <Label htmlFor="edit-philhealthNumber">
                                PhilHealth Number
                            </Label>
                            <Input
                        ...
                        <div>
                            <Label htmlFor="edit-pagibigNumber">
                                Pag-IBIG Number
                            </Label>
                            <Input
                                ...
                                onChange={e =>
                                    setGovernmentIdFormData({
                                        ...governmentIdFormData,
                                        pagibigNumber: formatPagIbigNumber(
                                            e.target.value
                                        ),
                                    })
                                }
                    ...
                    <DialogFooter>
                        ...
                        <Button
                            variant="outline"
                            onClick={() => setShowEditGovernmentIdDialog(false)}
                        >
                            Cancel
                        </Button>
                        <Button
            ...
            {/* Edit Leave Credit Defaults Dialog */}
            <Dialog
                open={showEditLeaveCreditsDialog}
                onOpenChange={setShowEditLeaveCreditsDialog}
            >
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>Edit Leave Credit Defaults</DialogTitle>
                        <DialogDescription>
                            Set the default leave credits for each leave type.
                        </DialogDescription>
                    </DialogHeader>

                    <div className="space-y-4">
                        {leaveCreditFormData.map((type, index) => (
                            <div key={type.leaveTypeId} className="space-y-2">
                                <Label
                                    htmlFor={`leave-credit-${type.leaveTypeId}`}
                                >
                                    {type.name}
                                </Label>
                                <Input
                                    id={`leave-credit-${type.leaveTypeId}`}
                                    type="number"
                                    min={0}
                                    step={1}
                                    value={type.defaultLimitDays}
                                    onChange={event => {
                                        const raw = Number(event.target.value)
                                        setLeaveCreditFormData(prev => {
                                            const next = [...prev]
                                            next[index] = {
                                                ...next[index],
                                                defaultLimitDays:
                    ...
                    <DialogFooter>
                        <Button
                            variant="outline"
                            onClick={() => setShowEditLeaveCreditsDialog(false)}
                            disabled={isLeaveCreditSaving}
                        >
                            Cancel
                        </Button>
                        <Button
                            onClick={() => {
                                void handleSaveLeaveCreditTypes()
                            }}
                            className="bg-vibrant-blue hover:bg-vibrant-blue/90"
                            disabled={isLeaveCreditSaving}
                        >
                            {isLeaveCreditSaving ? 'Saving...' : 'Save Changes'}
                        </Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
            ...
            >
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Configure biometric authentication settings for your
                            account
                        </DialogDescription>
                    </DialogHeader>
                    <div className="space-y-4">
                        ...
                        <div className="flex items-center justify-between">
                            <div className="flex-1">
                                ...
                                <p className="text-sm text-muted-foreground">
                                    Mark password changes as waived when
                                    applicable
                                </p>
                            </div>
                            <Switch
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="flex gap-2">
                            <Button
                                ...
                                disabled={
                                    validationCount !== 5 || isPasswordSaving
                                }

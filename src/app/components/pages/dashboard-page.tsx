...
export function DashboardPage() {
  ...
  const [filterStatus, setFilterStatus] = useState<"all" | "present" | "on-leave" | "absent">("all");
  ...
  const [isAdjustmentRequestsOpen, setIsAdjustmentRequestsOpen] = useState(false);
  
  // Adjustment requests counter
  const [adjustmentRequestsCount, setAdjustmentRequestsCount] = useState(0);
  const [adjustmentFilterStatus, setAdjustmentFilterStatus] = useState<"all" | "pending" | "approved" | "denied" | "cancelled">("pending");
  ...
  const [leaveRequests, setLeaveRequests] = useState<LeaveRequest[]>([
    ...
    },
    {
      ...
      attachments: [],
      logTrail: [
        { status: "pending", date: new Date(2025, 9, 17) },
      ],
    },
    {
      id: "lr-004",
    ...
    // Approved requests (3)
    {
      ...
      logTrail: [
        ...
        { status: "approved", date: new Date(2025, 8, 12), approvedBy: "Sarah Martinez" },
      ],
    },
    {
      id: "lr-007",
      ...
      logTrail: [
        ...
        { status: "approved", date: new Date(2025, 9, 9), approvedBy: "Michael Chen" },
      ],
    },
    {
      id: "lr-008",
      ...
      logTrail: [
        ...
        { status: "approved", date: new Date(2025, 8, 18), approvedBy: "Sarah Martinez" },
      ],
    },
    // Denied requests (3)
    {
      ...
      logTrail: [
        ...
        { status: "denied", date: new Date(2025, 9, 7), approvedBy: "Michael Chen", reason: "Insufficient leave credits. Only 5 days available, 8 days requested." },
      ],
    },
    {
      id: "lr-010",
      ...
      logTrail: [
        ...
        { status: "denied", date: new Date(2025, 8, 14), approvedBy: "Sarah Martinez", reason: "Emergency leave requires supporting documentation. Please provide necessary documents." },
      ],
    },
    {
      id: "lr-011",
      ...
      logTrail: [
        ...
        { status: "denied", date: new Date(2025, 9, 2), approvedBy: "Michael Chen", reason: "No overtime records found for the requested period. Please verify with your supervisor." },
      ],
    },
    // Cancelled requests (3)
    {
      ...
      logTrail: [
        ...
        { status: "approved", date: new Date(2025, 8, 26), approvedBy: "Sarah Martinez" },
        { status: "cancelled", date: new Date(2025, 9, 10), reason: "Plans changed - unable to proceed with leave" },
      ],
    },
    {
      id: "lr-013",
      ...
      logTrail: [
        ...
        { status: "cancelled", date: new Date(2025, 8, 28), reason: "Appointment rescheduled to a later date" },
      ],
    },
    {
      id: "lr-014",
      ...
      logTrail: [
        ...
        { status: "approved", date: new Date(2025, 8, 9), approvedBy: "Michael Chen" },
        { status: "cancelled", date: new Date(2025, 8, 11), reason: "Matter resolved - no longer need leave", attachments: [] },
      ],
    },
  ]);

  const [leaveFilterStatus, setLeaveFilterStatus] = useState<"all" | "pending" | "approved" | "denied" | "cancelled">("pending");
  const [selectedLeaveRequest, setSelectedLeaveRequest] = useState<LeaveRequest | null>(null);
  const [showLeaveDetailsDialog, setShowLeaveDetailsDialog] = useState(false);
  const [showApproveDialog, setShowApproveDialog] = useState(false);
  const [showDenyDialog, setShowDenyDialog] = useState(false);
  const [showCancelApprovedDialog, setShowCancelApprovedDialog] = useState(false);
  ...
  const exportToCSV = () => {
    const headers = ["Employee ID", "Last Name", "First Name", "Department", "Status", "Clock-In Time", "Clock-Out Time", "Work Duration", "Late"];
    const rows = sortedEmployees.map(emp => [
      ...
      emp.clockInTime || "N/A",
      emp.clockOutTime || (emp.status === "present" && !emp.clockOutTime ? "Active" : "N/A"),
    ...
    link.setAttribute("download", `employee-status-${new Date().toISOString().split('T')[0]}.csv`);
  ...
  const filteredLeaveRequests = leaveRequests.filter(req =>
    leaveFilterStatus === "all" ? true : req.status === leaveFilterStatus
      ...
      prev.map(req =>
        req.id === selectedLeaveRequest.id
          ? {
              ...
              logTrail: [
                ...req.logTrail,
                { status: "denied" as const, date: new Date(), approvedBy: "Sarah Martinez", reason: denyReason },
              ],
            }
          : req
      )
          ...
          ? {
              ...
              logTrail: [
                ...
                { 
                  ...
                  date: new Date(), 
                  reason: cancelReason,
                  ...(cancelAttachments.length > 0 ? { attachments: cancelAttachments } : {}),
                },
              ],
            }
          : req
      ...
      <div className="px-4 space-y-4">
        ...
        <Card>
          <CardHeader 
            ...
            onClick={() => setIsEmployeeStatusOpen(!isEmployeeStatusOpen)}
          >
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <span className="text-sm text-muted-foreground">
                  {sortedEmployees.length} {filterStatus !== "all" ? "Filtered" : "Employees"}
                </span>
                <motion.div
                  animate={{ rotate: isEmployeeStatusOpen ? 180 : 0 }}
                  ...
                  {employee.clockInTime && (
                    <div className="flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
                      <span className="flex items-center gap-1">
                        ...
                        In: {employee.clockInTime}
                      </span>
                      {employee.clockOutTime && (
                        <>
                          <span className="flex items-center gap-1">
                            Out: {employee.clockOutTime}
                          </span>
                          <span className="flex items-center gap-1 text-vibrant-blue">
                            Duration: {employee.workDuration}
                          </span>
                        </>
                      )}
                      {!employee.clockOutTime && employee.status === "present" && (
                        <span className="text-vibrant-green">Active</span>
                      )}
                    </div>
                  )}
                  {employee.status === "on-leave" && employee.workDuration && (
                    <div className="flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
        ...
        <Card>
          <CardHeader 
            className="pb-3 cursor-pointer hover:bg-accent/50 transition-colors"
            onClick={() => setIsLeaveRequestsOpen(!isLeaveRequestsOpen)}
          >
            <div className="flex items-center justify-between">
              <CardTitle className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-2">
                <span className={`text-sm font-medium ${
                  leaveFilterStatus === 'pending' ? 'text-vibrant-orange' :
                  leaveFilterStatus === 'approved' ? 'text-vibrant-green' :
                  leaveFilterStatus === 'denied' ? 'text-red-600' :
                  leaveFilterStatus === 'cancelled' ? 'text-muted-foreground' :
                  'text-muted-foreground'
                }`}>
                  {filteredLeaveRequests.length} Request{filteredLeaveRequests.length !== 1 ? 's' : ''}
                </span>
                <motion.div
                  animate={{ rotate: isLeaveRequestsOpen ? 180 : 0 }}
                  transition={{ duration: 0.3, ease: "easeInOut" }}
                >
                  <Button
                    variant="ghost"
          ...
          <AnimatePresence initial={false}>
            ...
            <div>
              ...
              <Select value={leaveFilterStatus} onValueChange={(value) => setLeaveFilterStatus(value as typeof leaveFilterStatus)}>
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
            ...
            <div className="space-y-2">
              {filteredLeaveRequests.length === 0 ? (
                <div className="text-center py-8 text-muted-foreground">
                  ...
                  <p>No {leaveFilterStatus !== "all" ? leaveFilterStatus : ""} leave requests found</p>
                </div>
              ) : (
                  ...
                  >
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1 flex-wrap">
                        ...
                        <Badge className={`text-xs ${getLeaveStatusBadge(request.status)}`}>
                          {request.status}
                        </Badge>
                      </div>
        ...
        <Card>
          <CardHeader 
            className="pb-3 cursor-pointer hover:bg-accent/50 transition-colors"
            onClick={() => setIsAdjustmentRequestsOpen(!isAdjustmentRequestsOpen)}
          >
            <div className="flex items-center justify-between">
              <CardTitle className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-2">
                <span className={`text-sm font-medium ${
                  adjustmentFilterStatus === 'pending' ? 'text-vibrant-orange' :
                  adjustmentFilterStatus === 'approved' ? 'text-vibrant-green' :
                  adjustmentFilterStatus === 'denied' ? 'text-red-600' :
                  adjustmentFilterStatus === 'cancelled' ? 'text-muted-foreground' :
                  'text-muted-foreground'
                }`}>
                  {adjustmentRequestsCount} Request{adjustmentRequestsCount !== 1 ? 's' : ''}
                </span>
                <motion.div
                  animate={{ rotate: isAdjustmentRequestsOpen ? 180 : 0 }}
                  transition={{ duration: 0.3, ease: "easeInOut" }}
                >
                  <Button
                    variant="ghost"
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      setIsAdjustmentRequestsOpen(!isAdjustmentRequestsOpen);
                    }}
                  >
                    <ChevronDown className="h-4 w-4" />
                  </Button>
              ...
              >
                <CardContent className="space-y-3 pt-0">
                  <AdjustmentRequestsSection 
                    onFilteredCountChange={setAdjustmentRequestsCount}
                    filterStatus={adjustmentFilterStatus}
                    onFilterStatusChange={setAdjustmentFilterStatus}
                  />
                </CardContent>
              </motion.div>
            )}
          ...
          <DialogContent className="max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle className="flex items-center justify-between">
                ...
                {selectedLeaveRequest && (
                  <Badge className={`${getLeaveStatusBadge(selectedLeaveRequest.status)}`}>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <div className="p-3 bg-muted rounded-md text-sm">
                      {format(selectedLeaveRequest.submittedDate, "MMM dd, yyyy")}
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <div className="p-3 bg-muted rounded-md text-sm">
                      {format(selectedLeaveRequest.startDate, "PPP")}
                    </div>
                  </div>
                  <div>
                    <Label>To Date</Label>
                    <div className="p-3 bg-muted rounded-md text-sm">
                      {format(selectedLeaveRequest.endDate, "PPP")}
                    </div>
                  </div>
                </div>
                ...
                {selectedLeaveRequest.attachments.length > 0 && (
                  <div>
                    ...
                    <div className="space-y-2 mt-2">
                      {selectedLeaveRequest.attachments.map((file, index) => (
                        <div
                          key={index}
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    {selectedLeaveRequest.logTrail.map((log, index) => (
                      <div
                        key={index}
            ...
            <DialogFooter>
              ...
              {selectedLeaveRequest?.status === "approved" && (
                <Button
                  variant="destructive"
                  onClick={() => setShowCancelApprovedDialog(true)}
                >
              ...
              <div className="px-6 pb-2">
                <div className="p-3 bg-muted rounded-lg text-sm">
                  <p className="font-medium text-foreground">
                    {selectedLeaveRequest.employeeName} - {selectedLeaveRequest.leaveType}
                  </p>
                  <p className="text-xs mt-1 text-muted-foreground">
                    {format(selectedLeaveRequest.startDate, "MMM dd")} -{" "}
                    {format(selectedLeaveRequest.endDate, "MMM dd, yyyy")}
                  </p>
                </div>
              </div>
            )}
        ...
        <Dialog open={showDenyDialog} onOpenChange={setShowDenyDialog}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                Please provide a reason for denying this leave request. This will be visible to the employee.
              </DialogDescription>
            </DialogHeader>
        ...
        <Dialog open={showCancelApprovedDialog} onOpenChange={setShowCancelApprovedDialog}>
          <DialogContent className="max-h-[90vh] overflow-y-auto">
            <DialogHeader>
                ...
                <div>
                  ...
                  <div className="mt-2 space-y-2">
                    <Button
                      ...
                      size="sm"
                      onClick={() => {
                        const fileName = prompt("Enter attachment filename:");
                        if (fileName) handleAddCancelAttachment(fileName);
                      }}
                      className="w-full"
                    >
                    ...
                    {cancelAttachments.map((file, index) => (
                      ...
                      >
                        ...
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => handleRemoveCancelAttachment(index)}

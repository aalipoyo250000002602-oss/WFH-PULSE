...
export function HomePage({
    ...
    interface AdjustmentRequestState extends Omit<
        AttendanceAdjustmentRequest,
        'status'
    > {
        status: AdjustmentStatus
    ...
    const [editingRequest, setEditingRequest] =
        useState<AdjustmentRequestState | null>(null)
    const [prefilledTimes, setPrefilledTimes] = useState<{
        clockIn: string
        clockOut: string
    } | null>(null)
    ...
    >([])
    const [isLoadingOvertimeRequests, setIsLoadingOvertimeRequests] =
        useState(false)
    ...
    const loadAdjustmentRequests = async () => {
        ...
        try {
            ...
            const rows = Array.isArray(payload?.requests)
                ...
                : []
            const mapped = rows
                .map(mapApiAdjustmentRequest)
                .filter(Boolean) as AdjustmentRequestState[]
    ...
    const upsertAdjustmentRequest = (request: AdjustmentRequestState) => {
        setAdjustmentRequests(prev => {
            const next = prev.filter(
                item => item.id !== request.id && item.date !== request.date
            )
            return [request, ...next]
        })
    }
        ...
        return {
            ...
            logs: Array.isArray(row.logs)
                ? row.logs.map((log: any) => ({
                      ...
                      status: String(log?.status ?? 'pending') as
                          'pending' | 'approved' | 'denied' | 'cancelled',
                      loggedAt: String(
                          log?.loggedAt ?? new Date().toISOString()
                      ),
                      approvedBy: log?.approvedBy
                          ? String(log.approvedBy)
                          : null,
    ...
    const loadOvertimeRequests = async () => {
        ...
        try {
            ...
            const rows = Array.isArray(payload?.requests)
                ...
                : []
            const mapped = rows
                .map(mapApiOvertimeRequest)
                .filter(Boolean) as OvertimeRequestState[]
    ...
    const handleRequestAdjustment = (date: string) => {
        ...
        const existingRequest =
            adjustmentRequests.find(req => req.date === date) ?? null
        ...
        if (
            details &&
            (details.status === 'present' || details.status === 'late')
        ) {
            setPrefilledTimes({
    ...
    const handleRequestOvertime = (date: string) => {
        ...
        if (!details || details.status !== 'present') {
            toast.error(
                'Overtime requests are only available for present dates.'
            )
            return
        }
        ...
        setSelectedDate(date)
        const existingRequest =
            overtimeRequests.find(req => req.requestDate === date) ?? null
    ...
    const handleSubmitAdjustment = async (
        ...
        try {
            ...
            if (!response.ok) {
                toast.error(
                    payload?.error || 'Unable to save adjustment request'
                )
                return false
            }
    ...
    const handleDeleteAdjustment = async (requestId: string) => {
        ...
        try {
            const response = await fetch(
                `${apiBaseUrl}/me/attendance-adjustments/${requestId}`,
                {
                    method: 'DELETE',
                    headers: {
                        Authorization: `Bearer ${accessToken}`,
            ...
            if (!response.ok) {
                toast.error(
                    payload?.error || 'Unable to delete adjustment request'
                )
                return false
            }
    ...
    const handleRevokeAdjustment = async (requestId: string) => {
        ...
        try {
            const response = await fetch(
                `${apiBaseUrl}/me/attendance-adjustments/${requestId}/revoke`,
                {
                    method: 'POST',
                    headers: {
                        Authorization: `Bearer ${accessToken}`,
            ...
            if (!response.ok) {
                toast.error(
                    payload?.error || 'Unable to revoke adjustment request'
                )
                return false
            }
    ...
    const handleDeleteOvertime = async (requestId: string) => {
        ...
        try {
            const response = await fetch(
                `${apiBaseUrl}/me/overtime-requests/${requestId}`,
                {
                    method: 'DELETE',
                    headers: {
                        Authorization: `Bearer ${accessToken}`,
            ...
            if (!response.ok) {
                toast.error(
                    payload?.error || 'Unable to delete overtime request'
                )
                return false
            }
    ...
    const handleRevokeOvertime = async (requestId: string) => {
        ...
        try {
            const response = await fetch(
                `${apiBaseUrl}/me/overtime-requests/${requestId}/revoke`,
                {
                    method: 'POST',
                    headers: {
                        Authorization: `Bearer ${accessToken}`,
            ...
            if (!response.ok) {
                toast.error(
                    payload?.error || 'Unable to revoke overtime request'
                )
                return false
            }
    ...
    return (
        <div className="space-y-6 pb-20">
            ...
            <AttendanceDetailsModal
                ...
                details={
                    selectedDate ? attendanceDetailsData[selectedDate] : null
                }
                ...
                adjustmentRequest={
                    selectedDate
                        ? ((adjustmentRequests.find(
                              ...
                              | ((typeof adjustmentRequests)[number] & {
                ...
                overtimeRequest={
                    selectedDate
                        ? (overtimeRequests.find(
                              request => request.requestDate === selectedDate
                          ) ?? null)
                        : null
                }
            />
            ...
            <AttendanceAdjustmentModal
                ...
                existingRequest={
                    editingRequest && editingRequest.status !== 'cancelled'
                        ? ({
                              ...editingRequest,

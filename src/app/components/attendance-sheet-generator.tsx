...
const generateLateDatesForEmployee = (employeeId: string): Set<number> => {
...
const generateNovemberAbsentDateForEmployee = (employeeId: string): number => {
    const empNum = parseInt(employeeId.replace('emp-', ''))
    let seed = empNum * 200
    const random = () => {
        seed = (seed * 9301 + 49297) % 233280
...
const generateNovemberLeaveDateForEmployee = (
    ...
    do {
        ...
        (lateDates.has(leaveDate) || leaveDate === absentDate) &&
        attempts < 20
    )
...
const getEmployeeLateDates = (employeeId: string): Set<number> => {
...
const getEmployeeNovemberLateDates = (employeeId: string): Set<number> => {
...
const getEmployeeNovemberAbsentDate = (employeeId: string): number => {
...
const getEmployeeNovemberLeaveDate = (
    ...
    if (!employeeNovemberLeaveDateCache.has(employeeId)) {
        employeeNovemberLeaveDateCache.set(
            employeeId,
            generateNovemberLeaveDateForEmployee(
                employeeId,
                lateDates,
                absentDate
            )
...
const generateAttendanceData = (employeeId: string, date: Date) => {
    ...
    const isOctober2025 = date.getFullYear() === 2025 && date.getMonth() === 9 // October is month 9 (0-indexed)
    const isNovember2025 =
        date.getFullYear() === 2025 &&
        date.getMonth() === 10 &&
        date.getDate() <= 15 // November 1-15
    ...
    if (isNovember2025) {
        ...
        const novemberLeaveDate = getEmployeeNovemberLeaveDate(
            employeeId,
            novemberLateDates,
            novemberAbsentDate
        )
...
export function AttendanceSheetGenerator({
    ...
    const [includeAttendanceReport, setIncludeAttendanceReport] =
        useState(false)
    ...
    const calculateSummary = () => {
        ...
        const summary = generatedReport.reduce(
            (acc, record) => {
                if (record.status === 'present') {
                    acc.presentDays++
                    // Parse work duration
                    const match = record.workDuration.match(/(\d+)h (\d+)m/)
                    if (match) {
        ...
        summary.totalWorkHours += Math.floor(summary.totalWorkMinutes / 60)
    ...
    `
        ...
        link.setAttribute(
            'download',
            `attendance-${selectedEmployee.id}-${format(dateRange.from, 'yyyyMMdd')}-${format(dateRange.to, 'yyyyMMdd')}.html`
        )
        link.style.visibility = 'hidden'
        document.body.appendChild(link)
        link.click()
    ...
    `
        ...
        link.setAttribute(
            'download',
            `payslip-${selectedEmployee.id}-${format(dateRange.from, 'yyyyMMdd')}-${format(dateRange.to, 'yyyyMMdd')}.html`
        )
        link.style.visibility = 'hidden'
        document.body.appendChild(link)
        link.click()
...
WFH PULSE`
    ...
    const handleSendEmail = () => {
        ...
        toast.success(
            `Payslip email sent to ${selectedEmployee.firstName} ${selectedEmployee.lastName}`
        )
    }

    const summary = calculateSummary()
    const payrollData = selectedEmployee?.payroll ? calculatePayroll() : null
                    ...
                    {showEmployeeList && (
                        <div className="border rounded-lg p-4 bg-muted/20">
                            <div className="flex items-center justify-between mb-3">
                                ...
                                <Select
                                    value={employeeListSort}
                                    onValueChange={(
                                        value: 'name' | 'department'
                                    ) => setEmployeeListSort(value)}
                                >
                            ...
                            <div className="mt-2 grid grid-cols-2 gap-2">
                                ...
                                <div className="text-center p-2 rounded-lg bg-vibrant-purple/10">
                                    <p className="text-sm font-bold text-vibrant-purple">
                                        {Math.floor(
                                            summary.totalLateMinutes / 60
                                        )}
                                        h {summary.totalLateMinutes % 60}m
                                    </p>
                                    <p className="text-xs text-muted-foreground">
                                        Total Late Time
                                    </p>
            ...
            <Dialog open={showEmailDialog} onOpenChange={setShowEmailDialog}>
                <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Compose and send the payslip to the employee via
                            email.
                        </DialogDescription>
                    </DialogHeader>
                        ...
                        <div className="flex items-center justify-between p-3 rounded-lg border">
                            <div className="flex items-center gap-2">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-muted-foreground">
                                        Attach the attendance report along with
                                        the payslip
                                    </p>
                                </div>
                            </div>
